...
- RMS 指标分配：系统运行支持模块的“装备RMS指标分配”本地工作台先在同一区域选择机型并下载/上传安装数数据，再在同一计算面板内按“RMS 输入参数 -> 指标分配方法 -> 执行计算”维护任务可靠度、任务时长、MTBF 和 MTTR 并发起计算，关键故障占比不再作为输入；独立导入的装备结构、安装数和运行比仍用于份额分配。计算方法保留等分配、比例分配和相似产品分配。安装数必须为正整数，运行比必须在 0..1；空结构、零总权重或无效相似基准均失败关闭。节点结果契约只包含层级、节点、型号、安装数、运行比、分配份额和状态，可导出真实 XLSX；页面不提供目标 R、校核可靠度、MTBCF、MTTR 裕度、保存或发布入口，也不影响装备系统建模中的 MTBF/MTTR 字段。装备系统建模页的模板下载、当前机型导出和文件上传统一位于左侧装备结构树面板，右侧详情面板只保留建模表与编辑器。
