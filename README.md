...
- 结果分析：独立“飞机任务可靠性评估”位于“任务可靠度评估”上方。该页面读取当前运行上下文中的飞机型号、基本任务剖面、任务时长、`reliabilityBlockDiagram` 和产品失效分布，用户点击“运行分析”后按 `R(t)=exp(-λt)`、串联乘积、并联 `1-∏(1-Ri)` 与 n 中取 k 动态规划自底向上计算整机可靠度；输入变化只清空旧结果，不静默重算。缺框图、缺关系/参数、无效时长/单位、循环或无效节点均失败关闭；页面只展示当前整机汇总并导出 XLSX，不提供保存、节点级明细或历史分析记录入口。备件短板分析、飞机转场携行清单、飞机任务可靠性评估、任务可靠度评估、停机因素分析和蒙特卡洛实验页均可把当前已完成的可见快照提交到后端 `/api/analysis-results/export-xlsx`，由 openpyxl 生成包含“分析信息 / 结果摘要 / 结果明细”的真实 OOXML；蒙特卡洛明细使用与页面相同的样本原始数量和实际比率。导出身份绑定在当前结果生成时的项目或实验方案上，无结果或运行中禁用导出，导出不重跑分析。任务可靠度摘要保留出动架次率、波次成功率、整周期任务可靠度、任务周期的 half-even 显示契约，并补充仿真总次数和成功次数；页面下半部分、波次成功率曲线及 XLSX 结果明细按波次位置聚合全部样本，展示第 i 个波次的平均成功率和参与样本数。任务可靠度按完整任务周期判定，`整周期任务可靠度 = 整周期成功实验数 / 已执行实验总数`，与基于产品结构的飞机任务可靠性是不同口径。停机因素按连续停机区间形成唯一事件账本，使用 `备件短缺 > 保障设备短缺 > 装备故障 > 预防性维修` 的直接原因优先级避免重复归类；四类汇总的次数、时长和当前筛选范围占比均从同一完整明细集合计算，页面多选筛选只更新展示而不重新运行仿真。用户可见页面、canonical 明细/快照和 XLSX 均不展示故障模式，仅保留任务、阶段、中文事件和 `DAY_n HH:MM` 时间上下文。
- 携行清单实际指标：实际即时满足率按 `Σimmediately_filled_quantity ÷ Σdemand_quantity`，实际备件利用率按 `Σconsumed_quantity ÷ Σcarried_quantity`；行级也使用同一分子分母，满足下限按实际即时满足率判断。projection 同时保留上述实际数量与 `projected_*` 规划字段，禁止互相兜底、平均行级/样本百分比或整数化实际数量。真实零分母显示 `--`，原始数量缺失/非法显示“数据不可用”；页面、详情和 Excel 使用相同口径。规划推荐算法保留，但以“规划/预计”名称单独展示。
- 携行清单满足率约束：用户设置的“备件满足率下限”必须进入后端 projection 计算，不得只在页面或结果指标中回显。对每个“机型 + 产品”，寻找最小整数携行量 `q`，使各成功样本需求 `dᵢ` 的预计满足量 `Σ min(dᵢ, q)` 不低于 `r × Σdᵢ`；零需求时满足率为 `1`。因此提高下限不得降低建议携行量；在满足下限的整数解中选择最小 `q`，作为“达标后备件利用率越高越好”的当前优化口径，利用率按约束后预计满足量除以全部成功样本的携行总量计算。projection、页面和 Excel 明细必须同时保留下限、约束后满足率、达标状态与约束余量。
