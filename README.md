...
- [`docs/windows-desktop-rc1.md`](docs/windows-desktop-rc1.md)：Electron + Docker Desktop/WSL2 一键安装版的构建、运行和发布边界。
...
- 结果分析：独立“飞机任务可靠性评估”位于“任务可靠度评估”上方。该页面读取当前运行上下文中的飞机型号、基本任务剖面、任务时长、`reliabilityBlockDiagram` 和产品失效分布，用户点击“运行分析”后按 `R(t)=exp(-λt)`、串联乘积、并联 `1-∏(1-Ri)` 与 n 中取 k 动态规划自底向上计算整机可靠度；输入变化只清空旧结果，不静默重算。缺框图、缺关系/参数、无效时长/单位、循环或无效节点均失败关闭；页面只展示当前整机汇总并导出 XLSX，不提供保存、节点级明细或历史分析记录入口。备件短板分析、飞机转场携行清单、飞机任务可靠性评估、任务可靠度评估和停机因素分析五个页面均显示页面标题，并在标题横幅右侧显示运行上下文；四个 lite Mesa 会话页同时保留分析设定、运行状态和显式“运行分析”入口，停机因素页保留排序范围。五页统一把当前已完成、已筛选和已排序的可见快照提交到后端 `/api/analysis-results/export-xlsx`，由 openpyxl 生成包含“分析信息 / 结果摘要 / 结果明细”的真实 OOXML；导出身份绑定在当前结果生成时的项目或实验方案上。无结果或运行中禁用导出，导出不重跑分析，蒙特卡洛实验页不提供此入口。任务可靠度摘要保留出动架次率、波次成功率、整周期任务可靠度、任务周期的 half-even 显示契约，并补充仿真总次数和成功次数；页面下半部分、波次成功率曲线及 XLSX 结果明细按波次位置聚合全部样本，展示第 i 个波次的平均成功率和参与样本数。任务可靠度按完整任务周期判定，`整周期任务可靠度 = 整周期成功实验数 / 已执行实验总数`，与基于产品结构的飞机任务可靠性是不同口径。停机因素按连续停机区间形成唯一事件账本，使用 `备件短缺 > 保障设备短缺 > 装备故障 > 预防性维修` 的直接原因优先级避免重复归类；四类汇总的次数、时长和当前筛选范围占比均从同一完整明细集合计算，页面多选筛选只更新展示而不重新运行仿真。用户可见页面、canonical 明细/快照和 XLSX 均不展示故障模式，仅保留任务、阶段、中文事件和 `DAY_n HH:MM` 时间上下文。
- 携行清单利用率：行级分子为同一机型与产品的 `spare_consumed` 事件数量，分母为“建议携行数量 × 成功执行样本数”，projection 以 `used_quantity` / `carried_quantity` 保留两项原始数量。总体备件利用率按 `Σ实际使用数量 ÷ Σ携行总数量 × 100%` 计算，禁止平均行级利用率，且始终基于未筛选结果全集；总携行量大于 0 时显示两位百分比（允许超过 100%），真实总携行量等于 0 时显示 `--`。空 projection 或原始数量缺失/非法属于契约数据不可用，不得用 aggregate、推荐量或行百分比反推，页面与 Excel 摘要显示“数据不可用”。行级有携行量但无使用显示 `0.00%`，零分母显示“不可计算”；页面与 Excel 摘要使用同一总体值，导出明细同时保留原始数量。
