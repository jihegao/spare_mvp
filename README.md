...
- 结果分析：独立“飞机任务可靠性评估”位于“任务可靠度评估”上方。该页面读取当前运行上下文中的飞机型号、基本任务剖面、任务时长、`reliabilityBlockDiagram` 和产品失效分布，用户点击“运行分析”后按 `R(t)=exp(-λt)`、串联乘积、并联 `1-∏(1-Ri)` 与 n 中取 k 动态规划自底向上计算整机可靠度；输入变化只清空旧结果，不静默重算。缺框图、缺关系/参数、无效时长/单位、循环或无效节点均失败关闭；页面只展示整机汇总并导出 XLSX，不提供保存或节点级明细，既有不可变历史快照仍可只读查看。备件短板分析、飞机转场携行清单、任务可靠度评估、停机因素分析仍是四个独立 lite Mesa 会话页，均展示运行上下文、分析设定、运行状态和显式“运行分析”入口；前三页继续不显示标题横幅，只有停机因素页恢复标题横幅并保留排序范围。任务可靠度按完整任务周期判定，`整周期任务可靠度 = 整周期成功实验数 / 已执行实验总数`，与基于产品结构的飞机任务可靠性是不同口径。停机因素按连续停机区间形成唯一事件账本，使用 `备件短缺 > 保障设备短缺 > 装备故障 > 预防性维修` 的直接原因优先级避免重复归类；四类汇总的次数、时长和当前筛选范围占比均从同一完整明细集合计算，页面多选筛选只更新展示而不重新运行仿真。
