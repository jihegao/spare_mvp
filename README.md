...
- 蒙特卡洛实验：导航入口直接进入嵌入的 Mesa 蒙特卡洛分析页，读取当前 Project JSON、样本量和随机种子并在页面内展示主要业务指标的均值、n-1 样本方差、单位和逐指标有效样本数，同时展示总/成功/失败样本数；旧 `monte-carlo-config`、列表和编辑 hash 兼容到该详情页。统计只接收白名单指标中的原生有限数值，失败样本、缺失值、字符串、NaN/Inf、样本身份、日志和调试字段不进入统计，n<2 时样本方差明确不可计算。
...
- RMS 指标分配：系统运行支持模块的“装备RMS指标分配”本地工作台在装备结构树上方按机型维护任务可靠度、任务时长、MTBF 和 MTTR，关键故障占比不再作为输入；独立导入的装备结构、安装数和运行比仍用于份额分配。计算方法保留等分配、比例分配和相似产品分配。安装数必须为正整数，运行比必须在 0..1；空结构、零总权重或无效相似基准均失败关闭。节点结果契约只包含层级、节点、型号、安装数、运行比、分配份额和状态，可导出真实 XLSX；页面不提供目标 R、校核可靠度、MTBCF、MTTR 裕度、保存或发布入口，也不影响装备系统建模中的 MTBF/MTTR 字段。
- 结果分析：独立“飞机任务可靠性评估”位于“任务可靠度评估”上方。该页面读取当前运行上下文中的飞机型号、基本任务剖面、任务时长、`reliabilityBlockDiagram` 和产品失效分布，用户点击“运行分析”后按 `R(t)=exp(-λt)`、串联乘积、并联 `1-∏(1-Ri)` 与 n 中取 k 动态规划自底向上计算整机可靠度；输入变化只清空旧结果，不静默重算。缺框图、缺关系/参数、无效时长/单位、循环或无效节点均失败关闭；页面只展示整机汇总并导出 XLSX，不提供保存或节点级明细，既有不可变历史快照仍可只读查看。备件短板分析、飞机转场携行清单、飞机任务可靠性评估、任务可靠度评估和停机因素分析五个页面均显示页面标题，并在标题横幅右侧显示运行上下文；四个 lite Mesa 会话页同时保留分析设定、运行状态和显式“运行分析”入口，停机因素页保留排序范围。任务可靠度按完整任务周期判定，`整周期任务可靠度 = 整周期成功实验数 / 已执行实验总数`，与基于产品结构的飞机任务可靠性是不同口径。停机因素按连续停机区间形成唯一事件账本，使用 `备件短缺 > 保障设备短缺 > 装备故障 > 预防性维修` 的直接原因优先级避免重复归类；四类汇总的次数、时长和当前筛选范围占比均从同一完整明细集合计算，页面多选筛选只更新展示而不重新运行仿真。
...
- Lite Mesa 当前运行契约：用户可见的 Monte Carlo 和四个结果分析页以 `POST /api/mesa-analysis-runs` 为主路径。请求携带当前 Project 或 ExperimentPlan 分支 Project、分析类型和页面设置；后端编译为 `aircraft_support_v1` simulation inputs，在内存中运行 `AircraftSupportV1Model` 样本，并返回 `session_complete`、指标、表格、事件快照和限制说明。每个样本默认有 60 秒硬上限，整批预算按 `ceil(samples / worker_count)` 个执行波次计算并限制在 180 到 900 秒；前端请求上限使用同一波次口径并额外保留 30 秒响应余量。响应中的请求/完成/失败样本数、样本诊断和编译/执行/聚合/投影耗时用于区分正常长任务、样本失败和超时。可视化推演页由 Solara iframe 内的 Mesa 控制器直接推进模型；该 iframe 承载不写入 Project，不创建运行账本，不生成持久化 artifact，也不读取 current-result 面板。#261 的定位与 `case-large` 性能证据见 [`reports/2026-07-18-issue-261-monte-carlo-timeout/README.md`](reports/2026-07-18-issue-261-monte-carlo-timeout/README.md)。
