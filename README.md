...
- M2a/M9.5 契约适配：`src/spare_mvp_contract/adapter.py` 已支持 Project JSON 校验、已批准的 `smoke` Scenario 编译、`SmokeSpareMvpModel` 运行，以及 `aviation_support` Scenario 编译、`AviationSupportModel` 单次正式执行和受治理采样契约下的 `aviation_support` formal Monte Carlo；正式 run 都写出 Result summary、ArtifactManifest 和可追溯 artifacts。
