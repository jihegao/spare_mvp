...
- 可视化仿真：导航入口直接进入 Solara iframe 承载的 Mesa 页面，不再经过可视化启动/回放列表页；旧 `visual-start-stop`、`scenario-switch`、`visual-results` hash 兼容到该页面。`scripts/start-system.sh` 默认启动平台管理的 Solara 可视化 sidecar（默认 `http://127.0.0.1:8765/`），页面 iframe 可通过 `solaraUrl` 参数或 localStorage 覆盖地址。该 sidecar 只作为交互式可视化承载，不恢复旧 contract provider、`independent-mesa` 旁路、旧 `/api/runs` 运行账本主流程、状态流或 artifact 回放；Monte Carlo 和四个分析页仍以 lite Mesa 会话摘要为用户可见结果来源。Mesa 内部 `Ontology视图`、Ontology Playground 导出和项目级本体校验已从当前产品范围删除。
- 蒙特卡洛实验：导航入口直接进入嵌入的 Mesa 蒙特卡洛分析页，读取当前 Project JSON、样本量和随机种子并在页面内展示统计结果；旧 `monte-carlo-config`、列表和编辑 hash 兼容到该详情页。
- 仿真实验方案流程：ExperimentPlan 继续承载分支 Project JSON、样本量、随机种子和页面会话设置；当前用户可见的运行结果由 `当前 Project -> POST /api/mesa-analysis-runs -> aircraft_support_v1 simulation inputs -> in-memory AircraftSupportV1Model -> lite Mesa 会话摘要` 生成。旧 `RunIntent -> /api/runs -> RunService -> SimulationAdapter -> SQLite + artifacts` 账本链路保留为历史实现、内部治理能力和后续持久化运行治理候选，不再作为可视化、Monte Carlo 或四类分析页的用户主流程结果来源。旧 `smoke` 模型、contract provider、smoke scenarios 和 smoke JSON fixtures 已退役删除；`aviation_support` 仅保留历史 schema/fixture 证据且 adapter 入口返回 `retired_model_family` 并指向 `aircraft_support_v1`。`aircraft_support_v1` 是当前正式模型核心；`components[].failureDistribution` 是 behavior-driving 故障分布输入，其他行为驱动字段覆盖机队数量/初始可用、任务波次、组件寿命/RMS/k-out-of-n、`reliabilityBlockDiagram`、`supportResources[]` 人员/设备/备件数量、顶层 `transportPolicies[]`、保障活动 job DAG、周期任务、任务阶段/机场、Monte Carlo sweep 与 seed。后勤保障活动建模页读写 `scenario.transportPolicies`，不再把运输策略保存为 `supportActivities[].transportStrategies`；旧活动级策略仅作为导入/保存兼容来源被提升到顶层。`supportOrganization` 当前批准为 governance-only / 不驱动仿真字段，进入 provenance 而不阻断 M9.6 冻结案例。会话结果是当前规则和参数下的原型分析摘要，不是工程级校准结论。
- RMS 指标分配：系统运行支持模块的“装备RMS指标分配”本地工作台以独立导入的装备结构、安装数和运行比为输入，左侧展示装备结构树，右侧展示导入安装数表；计算方法作为独立区块紧邻节点分配结果，保留等分配、比例分配和相似产品分配。安装数必须为正整数，运行比必须在 0..1；空结构、零总权重或无效相似基准均失败关闭。节点结果契约只包含层级、节点、型号、安装数、运行比、分配份额和状态，可导出真实 XLSX；页面不再反算或展示目标 R、校核可靠度、MTBCF、MTBF、MTTR 裕度，也不提供保存或发布入口，不影响装备系统建模中的 MTBF/MTTR 字段。
- 结果分析：新增独立“飞机任务可靠性评估”，位于“任务可靠度评估”上方。该页面读取当前 Project 的飞机型号、基本任务剖面、任务时长、`reliabilityBlockDiagram` 和产品失效分布，按 `R(t)=exp(-λt)`、串联乘积、并联 `1-∏(1-Ri)` 与 n 中取 k 动态规划自底向上计算整机可靠度；缺框图、缺关系/参数、无效时长/单位、循环或无效节点均失败关闭。结果明细覆盖整机、系统、分系统和产品，可导出 CSV，并通过 `POST/GET /api/projects/{project_id}/aircraft-mission-reliability-analyses` 保存和查看不可变历史快照。备件短板分析、飞机转场携行清单、任务可靠度评估、停机因素分析仍是四个独立 lite Mesa 会话页；其中任务可靠度按完整任务周期判定，`整周期任务可靠度 = 整周期成功实验数 / 已执行实验总数`，与基于产品结构的飞机任务可靠性是不同口径。
...
- PR-D 数据持久化：`src/spare_mvp_backend/schema.sql` 和 repository helper 已提供 SQLite 版 Project / Scenario / Run / Result / ArtifactManifest 持久化与 `run_id` 身份链查询；飞机任务可靠性分析另存为项目级不可变快照，不写入 clean Project 结果字段。`reliabilityBlockDiagram` 本身属于建模输入并保留在 clean Project。历史运行账本对象当前定位为历史实现、内部治理能力或后续持久化运行治理候选。
