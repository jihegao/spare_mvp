...
- RMS 指标分配：系统运行支持模块的“装备RMS指标分配”本地工作台先选择机型并下载/上传安装数数据，再维护任务可靠度、任务时长、整机 MTBF 和 MTTR，按等分配、比例分配或相似产品分配规则计算节点权重。引擎以整机故障风险预算换算各节点失效率和 MTBF，并按维修难度缩放节点 MTTR；页面与真实 XLSX 统一展示层级、节点、运行比、失效率、MTBF、MTTR 六列。安装数必须为正整数，运行比必须在 0..1；运行比为 0 的节点不参与风险分配，空结构、零总权重或无效相似基准均失败关闭。页面不提供目标 R、校核可靠度、MTBCF、MTTR 裕度、保存或发布入口，也不影响装备系统建模中的 MTBF/MTTR 字段。装备系统建模页的模板下载、当前机型导出和文件上传统一位于左侧装备结构树面板，右侧详情面板只保留建模表与编辑器。
