...
- 结果分析：备件短板分析、飞机转场携行清单、飞机任务可靠性分析、停机因素分析；M6.2 起 formal-result boundary 会读取 run status、ResultSummary `analysis_outputs` 和 ArtifactManifest projection metadata，只有对应 projection artifact 存在时才显示正式后端结果状态；缺少 compiler provenance 或官方 analysis artifact 时仍只显示前端本地预览标签。
...
- M6.2 统一 Monte Carlo / analysis profile：`front/api-client.mjs` 会把 ExperimentPlan 分支 Project JSON、Monte Carlo sweep 和 `analysisRequests` 一起提交；`RunService` 允许 `run_type=monte_carlo`，复用同一 Scenario compiler gate，成功后写入 `monte_carlo_base`、`large_sample_summary`、`spare_shortfall`、`carry_list`、`mission_reliability`、`downtime_factors` artifacts。该切片仍是同步本地执行器和 manifest metadata 展示，不包含生产 worker queue、object storage、取消/重试、artifact payload 浏览 API 或新的 auth/audit scope。
