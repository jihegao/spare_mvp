...
- 可视化仿真：单次仿真的任务态势、机场保障视图、指标和事件流；M9.8 后平台入口默认使用 `aircraft_support_v1` 通过 canonical `/api/runs` 提交正式 run，并从 `visualization_state_series` artifact 或 `/api/runs/{run_id}/state-stream` 展示状态。Mesa 内部 `Ontology视图`、Ontology Playground 导出和项目级本体校验已从当前产品范围删除。
