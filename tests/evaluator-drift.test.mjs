...
test("schema manifest traces Result contracts to normalized model snapshots", async () => {
  const manifest = await readJson("contracts/README.md.json");
  assert.ok(manifest.source_inputs.includes("Simulation Adapter normalized Result derived from model.snapshot()"));
...
test("scenario adapter mapping covers every compiled simulation input for each model family", async () => {
  ...
  assert.equal(mapping.model_families.aviation_support.simulation_inputs.lru_failure_multiplier.constructor_param, "lru_failure_multiplier");
