...
test("carry list result exposes satisfaction, zero-demand, life-limit, and aircraft UI", async () => {
  const runtime = await setupRuntimeApp({
    ...
    liteMesaAnalysisResponseOverrides: {
      ...
      rows: [
        { aircraftModel: "J-15", productId: "product-engine", spareType: "不得显示的旧发动机备件", recommended: 5, usedQuantity: 1, carriedQuantity: 1, demand: 6, shortage: 1, utilization: 0, riskLevel: "高", lifeLimited: true, lifeLandings: 120, lifeHours: 240 },
        { aircraftModel: "J-35", productId: "product-radar", spareType: "不得显示的旧雷达备件", recommended: 2, usedQuantity: 1, carriedQuantity: 9, demand: 2, shortage: 0, utilization: 2, riskLevel: "低", lifeLimited: false, lifeLandings: 0, lifeHours: 0 },
        { aircraftModel: "J-15", productId: "product-zero", spareType: "不得显示的旧零需求备件", recommended: 0, usedQuantity: 0, carriedQuantity: 0, demand: 0, shortage: 0, riskLevel: "低", lifeLimited: false, lifeLandings: 0, lifeHours: 0 }
      ]
    }
  });
  ...
  try {
    ...
    assert.match(detailPanel, /发动机控制模块 \/ EC-15[\s\S]*<td>100\.00%<\/td>/);
    assert.match(detailPanel, /雷达组件 \/ RD-35[\s\S]*<td>11\.11%<\/td>/);
...
test("carry list overall utilization is unavailable when total carried quantity is zero", async () => {
  const runtime = await setupRuntimeApp({
    hash: "feature=spare-planning-carry-list-analysis",
    liteMesaAnalysisResponseOverrides: {
      rows: [
  ...
  try {
    await runtime.click("[data-lite-mesa-analysis-action='run']");
    const detailPanel = htmlSectionByClass(runtime.appNode.innerHTML, "lite-mesa-analysis-detail");
    assert.match(detailPanel, /总体备件利用率[\s\S]*--/);
    assert.doesNotMatch(detailPanel, /NaN|Infinity/);
...
test("carry list does not treat missing or invalid raw quantities as a real zero denominator", async () => {
  const runtime = await setupRuntimeApp({
    hash: "feature=spare-planning-carry-list-analysis",
    liteMesaAnalysisResponseOverrides: {
      rows: [
...
test("carry list Excel export follows aircraft, zero-demand, and recommended-quantity sorting", async () => {
  const runtime = await setupRuntimeApp({
    ...
    liteMesaAnalysisResponseOverrides: {
      rows: [
        { aircraftModel: "J-15", productId: "carry-a", recommended: 4, usedQuantity: 8, carriedQuantity: 4, demand: 4, shortage: 1, utilization: 0, lifeLimited: true, lifeLandings: 100, lifeHours: 0, riskLevel: "高" },
        { aircraftModel: "J-15", productId: "carry-b", recommended: 2, usedQuantity: 0, carriedQuantity: 0, demand: 3, shortage: 0, utilization: 2, lifeLimited: false, riskLevel: "低" },
        { aircraftModel: "J-15", productId: "carry-zero", recommended: 0, usedQuantity: 0, carriedQuantity: 0, demand: 0, shortage: 0, lifeLimited: false, riskLevel: "低" },
        { aircraftModel: "J-16", productId: "carry-a", recommended: 1, usedQuantity: 0, carriedQuantity: 1, demand: 1, shortage: 0, lifeLimited: false, riskLevel: "低" }
      ]
    }
  });
  try {
    ...
    assert.deepEqual(body.detail_sections[0].columns, ["机型", "产品", "建议携行数量", "实际使用数量", "携行总数量", "需求次数", "短缺次数", "备件利用率", "有寿件", "起落寿命", "使用寿命(h)", "优先级"]);
    assert.deepEqual(body.detail_sections[0].rows.map((row) => [row[0], row[1], row[2], row[3], row[4], row[7], row[8]]), [
      ["J-15", "液压泵 / B-01", 2, 0, 0, "不可计算", "否"],
      ["J-15", "航电模块 / A-01", 4, 8, 4, "200.00%", "是"]
    ]);
...
test("RMS method selection updates method-specific parameters at runtime", async () => {
  ...
  try {
    ...
    assert.match(calculationPanel, /data-rms-basic-mission/);
    assert.match(calculationPanel, /data-rms-path="inputs\.missionReliability"/);
    assert.match(calculationPanel, /data-rms-path="basicMission\.missionHours"[^>]*disabled/);
    assert.doesNotMatch(calculationPanel, /data-rms-path="inputs\.mtbfHours"/);
...
test("RMS method changes invalidate the current aircraft result until recalculation completes", async () => {
  ...
  try {
    ...
    assert.match(initialResultPanel, /<th>本层份额<\/th><th>失效率<\/th><th>MTBF\(h\)<\/th><th>MTTR\(h\)<\/th>/);
    assert.match(initialResultPanel, /data-rms-result-field="mtbfHours"/);
    assert.match(initialResultPanel, /计算完成，分解结果尚未保存/);
...
test("RMS calculated result remains unsaved until the explicit result save action", async () => {
  const runtime = await setupRuntimeApp({
    hash: "feature=system-management-equipment-rms-allocation",
    projectJson: createRmsRuntimeProjectJson()
  });
  try {
    ...
    await runtime.click("[data-rms-action]", { rmsAction: "calculate" });
    await new Promise((resolve) => setTimeout(resolve, 2050));
    assert.match(runtime.appNode.innerHTML, /计算完成，分解结果尚未保存/);
    assert.equal(projectSaveBodies(runtime).at(-1)?.rmsAllocationResult?.byAircraftModel?.["J-15"], undefined);

    await runtime.click("[data-rms-action]", { rmsAction: "save-result" });
    const saved = await waitForProjectSave(runtime, (body) => Boolean(
      body.rmsAllocationResult?.byAircraftModel?.["J-15"]?.savedAt
      && Object.keys(body.rmsAllocationResult?.byContext || {}).length === 1
    ), "expected explicitly saved RMS result context");
    assert.equal(saved.rmsAllocationResult.byAircraftModel["J-15"].source, "calculated");
    assert.match(runtime.appNode.innerHTML, /分解结果已保存/);
  } finally {
    runtime.restore();
  }
});
...
test("RMS calculation is blocked without an aircraft or with invalid required inputs", async () => {
  ...
  try {
    ...
    await runtime.change("[data-rms-aircraft-model]", {}, { value: "J-15" });
    await runtime.change("[data-rms-path]", { rmsPath: "inputs.missionReliability" }, { value: "1", type: "number" });
    await runtime.click("[data-rms-action]", { rmsAction: "calculate" });
    assert.match(runtime.appNode.innerHTML, /整机任务可靠度 R\(T\)必须小于 1/);
    assert.doesNotMatch(runtime.appNode.innerHTML, /rms-calculation-overlay/);
  } finally {
    runtime.restore();
  }
...
test("RMS per-aircraft inputs, tree selection and saved results hydrate without cross-model reuse", async () => {
  ...
  try {
    ...
    assert.match(runtime.appNode.innerHTML, /data-rms-calculation-status="completed"[^>]*>计算完成/);
    assert.match(
      htmlSectionByClass(runtime.appNode.innerHTML, "rms-calculation-panel"),
      /data-rms-path="inputs\.missionReliability"/
    );
    ...
    assert.match(restoredResultPanel, /<th>本层份额<\/th><th>失效率<\/th><th>MTBF\(h\)<\/th><th>MTTR\(h\)<\/th>/);
    assert.match(restoredResultPanel, /data-rms-action="save-result"/);
    ...
    await runtime.change("[data-rms-path]", { rmsPath: "inputs.missionReliability" }, { value: "0.9", type: "number" });
    await new Promise((resolve) => setTimeout(resolve, 850));
    ...
    assert.ok(Math.abs(saved.rmsAllocationResult.byAircraftModel["J-15"].totals.missionReliability - 0.91) < 1e-9);
...
test("RMS input changes clear only the current aircraft result and block empty export", async () => {
  ...
  try {
    assert.match(htmlSectionByClass(runtime.appNode.innerHTML, "rms-result-panel"), /<th>失效率<\/th>/);
    await runtime.change("[data-rms-path]", { rmsPath: "inputs.missionReliability" }, { value: "0.925", type: "number" });
...
test("legacy operations phases persist independent references while sharing one basic definition", async () => {
  const projectId = "operations-selection-materializes-independent-phases";
  const projectJson = createRuntimeProjectJson({ project_id: projectId });
  projectJson.supportActivities[0].planType = "使用保障方案";
  delete projectJson.supportActivities[0].planGroupId;
  const runtime = await setupRuntimeApp({
    projectJson,
    backendProjects: [runtimeBackendProjectEntry(projectId, "三阶段独立引用")]
  });
  ...
  try {
    ...
    await runtime.click("[data-support-activity-job-add]", { supportActivityJobAdd: "ops_relaunch" });
    await runtime.click("[data-support-activity-job-template]", {
      supportActivityJobTemplate: "ops_relaunch",
    ...
    const saved = await waitForProjectSave(runtime, (body) => (
      body.supportActivities?.filter((activity) => activity.planGroupId === "ops-runtime-1").length === 3
    ), "expected one persisted three-phase operations plan");
    const phases = new Map(
      saved.supportActivities
        .filter((activity) => activity.planGroupId === "ops-runtime-1")
        .map((activity) => [activity.planType, activity])
    );
    assert.deepEqual(phases.get("直接准备方案").activityCodes, ["BA-001"]);
    assert.deepEqual(phases.get("再次出动准备方案").activityCodes, ["BA-001"]);
    assert.deepEqual(phases.get("飞行后检查方案").activityCodes, []);
    assert.equal(saved.supportActivityJobs.filter((job) => job.activityCode === "BA-001").length, 1);
  } finally {
    runtime.restore();
  }
});
...
test("basic support activity scope edits persist on the selected top-level job", async () => {
  ...
  try {
    ...
    const savedProject = await waitForProjectSave(runtime, (body) => (
      body.supportActivities?.length === 3
      && body.supportActivities.find((activity) => activity.planType === "直接准备方案")
        ?.activityCodes?.join(",") === "BA-001,BA-002"
    ...
    const directPreparation = savedProject.supportActivities.find(
      (activity) => activity.planType === "直接准备方案"
    );
    assert.equal(directPreparation.aircraftModel, "J-15");
    assert.deepEqual(directPreparation.activityCodes, ["BA-001", "BA-002"]);
    assert.deepEqual(
      savedProject.supportActivities
        .filter((activity) => activity.planType !== "直接准备方案")
        .map((activity) => activity.activityCodes),
      [[], []]
    );
  } finally {
    runtime.restore();
...
test("corrective maintenance view uses selected component activities and MTTR", async () => {
  ...
  try {
    ...
    assert.match(runtime.appNode.innerHTML, /换件比例为 0%/);
    await runtime.change(
      "[data-maintenance-replacement-ratio]",
      { maintenanceReplacementRatio: "1" },
    ...
    const saved = await waitForProjectSave(runtime, (body) => (
      ...
      && body.supportActivities?.[1]?.replacementRatio === 1
    ), "expected canonical maintenance method fields to persist");
    assert.equal(saved.supportActivities[1].repairType, undefined);
...
test("preventive maintenance view edits the same canonical maintenance method pair", async () => {
  const projectId = "project-preventive-method-runtime";
  const projectJson = createRuntimeProjectJson({ project_id: projectId });
  projectJson.supportActivities.push({
    id: "preventive-j15",
    activityType: "预防性维修",
    ...
    aircraftModel: "J-15",
    maintenanceMethods: ["non_replacement", "replacement"],
    replacementRatio: 0.25,
    activityCodes: [],
    predecessors: {}
  });
  const runtime = await setupRuntimeApp({
    projectJson,
    backendProjects: [runtimeBackendProjectEntry(projectId, "预防性维修方式运行时项目")]
  });
  ...
  try {
    ...
    await runtime.click("[data-project-draft-save]");
    const saved = await waitForProjectSave(runtime, (body) => (
      body.supportActivities?.[1]?.replacementRatio === 0.4
    ), "expected preventive maintenance ratio to persist");
    assert.deepEqual(saved.supportActivities[1].maintenanceMethods, ["non_replacement", "replacement"]);
  } finally {
    runtime.restore();
  }
});
...
function createPersistedRmsRuntimeProjectJson() {
  ...
  j15Plan.schemaVersion = "rms-allocation-plan-v4";
  j15Plan.algorithmVersion = "rms-engine-4.1.0";
  j15Plan.methods.allocation = "proportional";
  j20Plan.inputs.missionReliability = 0.88;
  j20Plan.schemaVersion = "rms-allocation-plan-v4";
  j20Plan.algorithmVersion = "rms-engine-4.1.0";
