...
2. Remember the modeling table structure.
   ...
   - Treat `supportOrganization.tree` as one canonical root object and persist `supportOrganization.runtimeMode` as `legacy`, `vertical`, or `vertical_lateral`. Nodes own complete four-dimensional `serviceScope`; `supportOrganization.relations[]` contains only same-parent lateral DAG edges and persists `enabled`. `supportNodes[]` and every `supportResources[]` row use `organizationNodeId`, while transport endpoints use `fromOrganizationNodeId` / `toOrganizationNodeId`. `vertical_lateral` uses local, enabled direct incoming lateral, then parent-chain candidates; `vertical` retains #315 without lateral. A one-element legacy root array and unique name aliases are migration inputs only.
   - Treat `organization_graph_identity` as the cross-output identity for the compiled organization graph. Runtime organization facts separate local fulfillment, candidate selection/rejection, dispatch, arrival and blocking; selected remote supply is not fulfilled until arrival. The descriptive summary uses per-sample request keys and must not be presented as causal analysis. Historical top-level transport policies left in `legacy` mode require an explicit migration notice before vertical/lateral activation.
...
- Treat `supportNodes`, `supportResources`, and `supportOrganization` as a validated allocation graph, not display-only metadata. The independent compiler emits both `support_network.nodes[].organization_node_id` and the canonical `organization_graph.runtime_mode`. `vertical` drives local/parent-chain supply; `vertical_lateral` inserts enabled direct incoming lateral candidates ordered by relation priority/id/source before the parent chain. Never infer siblings without an edge, reverse an edge, traverse multiple lateral hops, or use a global pool. Lateral scope must match resource/product/aircraft/destination airport. Retain explicit `supportActivities[].resourceId`; default it only when the root organization uniquely maps one runtime node, otherwise fail closed. Missing personnel/equipment resources mean zero capacity.
- When checking organization observability, use `python3 scripts/export-organization-observability-cases.py --check` from the repository root. The fixture is model output, not a new scheduling algorithm; it must retain lateral success, vertical fallback and fail-closed cases under seed 317.
...
- Treat `components` and `combatUnit.members` as equipment structure and behavior inputs. A hierarchy that points children at `aircraft-root` must persist that ID as one real, parentless whole-aircraft component; the UI may hide the row but the compiler must be able to resolve every `parentId` and non-empty activity `equipmentId`. Use `failureDistribution` / `repairDistribution` instead of restored component-level legacy rates or repair-ratio fields. For failure distributions, accept both legacy `parameters` strings and the current editor fields: exponential `rate`/`lambda`, normal `mean`, uniform `min`/`max`, and fixed `value`/`mean`.
