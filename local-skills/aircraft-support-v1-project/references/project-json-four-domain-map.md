...
- Corrective and preventive activities own the canonical pair `maintenanceMethods` (`non_replacement`, `replacement`) and `replacementRatio` (0-1). A historical activity missing both fields means non-replacement only with ratio 0; legacy `repairType` is accepted only for exact `原位维修` / `换件维修` migration and must not coexist with conflicting canonical values.
