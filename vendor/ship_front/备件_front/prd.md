...
| 当前交付形态 | 静态前端原型，包含首页导航和 4 个结果分析页面 |
...
当前前端原型聚焦结果分析层，参考既有“保障方案分析”类页面形态，提供模块导航、启动分析、结果展示、表格和图形化呈现能力。
...
| 备件规划评估模块 | 转场携行清单分析 | 基于优化条件和约束条件生成携行清单 |
...
参考文档描述，转场携行清单分析用于聚合携行备件数据，输出各备件配置数量、备件满足率、平均延误时间等指标，并展示保障能力差异。当前原型将参数配置调整为“优化条件 + 约束条件”模式。
...
| 备件满足率不低于 | 数值输入 | 0 到 1，默认 0.90 | 是 | 作为清单生成约束 |
| 备件利用率不低于 | 数值输入 | 0 到 1，默认 0.70 | 是 | 作为清单生成约束 |
...
3. 约束条件固定展示“备件满足率不低于”和“备件利用率不低于”两个输入项。
4. 满足率较低的备件应被标记为更高携行优先级。
5. 输出结果需说明当前优化条件和约束条件。
...
| --- | --- |
| 约束输入为空 | 使用默认值或提示用户补充 |
| 约束值小于 0 或大于 1 | 提示范围错误，不允许启动真实计算 |
| 无可用备件数据 | 展示空表格提示 |
| 计算服务异常 | 保留上一轮结果并提示启动失败 |

#### 验收标准

1. 参数区展示 1 个优化条件下拉框和 2 个约束数值输入框。
2. 优化条件包含使用可用度、出动架次率、再次出动准备时间。
3. 页面展示备件满足率不低于、备件利用率不低于两个约束项。
4. 修改优化条件后，结果指标卡同步更新。
5. 点击“启动”后有启动反馈。
...
1. [待确认] 转场携行清单分析中两个约束默认值是否固定为 0.90 和 0.70。
2. [待确认] “使用可用度、出动架次率、再次出动准备时间”是否只允许单选，还是后续需要支持多目标权重。
3. [待确认] 三个无参数页面是否在正式系统中完全不需要隐藏参数，还是仅当前原型阶段不展示。
4. [待确认] 启动按钮后续是否需要对接真实计算服务、进度状态和计算日志。
5. [待确认] 图表展示样式是否沿用当前静态原型，还是需要与最终平台组件库统一。
