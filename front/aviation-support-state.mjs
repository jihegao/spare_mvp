...
export function normalizeAviationSupportState(state = AVIATION_SUPPORT_DEMO_STATE) {
  ...
  return {
    ...
    missions: (state.missions || []).map((item) => ({
      id: item.mission_id,
      name: item.name || item.basic_task_name || item.mission_id,
      taskCategory: item.task_category || "",
      periodicTaskName: item.periodic_task_name || "",
      compositeTaskName: item.composite_task_name || "",
      basicTaskName: item.basic_task_name || item.name || item.mission_id,
      requiredAircraftType: item.required_aircraft_type || item.aircraft_type || "",
      groupName: item.group_name || "",
      waveIndex: number(item.wave_index || 1),
