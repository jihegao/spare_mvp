...
| `npm test` | 90 个 Node contract/frontend/ontology 测试通过，0 失败。 |
...
9. `get_run_chain` 验证 project -> scenario -> run -> result summary -> artifact manifest 身份链。
...
- `run_id` 当前由 smoke scenario identity 派生，适合此合约回归 smoke，不代表未来多次运行的唯一性策略。
