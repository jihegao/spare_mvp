...
6. 可视化推演页面直接显示 Mesa 页面内容，不显示外层四级导航，也不显示旧 `Ontology视图`；页面保留当前项目/已冻结方案运行上下文和单轨迹 session 参数，当前项目实时编译不得写回 Project。
7. 当前同源后端验证用 `npm run start:system` 启动后，浏览器从 `/front/` 通过 `/api/mesa-analysis-runs` 提交当前 Project draft 或已冻结 ExperimentPlan identity，返回 lite Mesa 会话指标、表格和事件摘要；可视化通过 `/api/visualization-sessions` 创建临时单轨迹输入并交给 Solara。M3-1 `/api/runs` smoke 闭环只作为归档证据保留在 `reports/m3-1-browser-backend-smoke/`。
...
16. 四个结果分析页是独立 `lite-mesa-analysis` 会话页：页面直接提交当前 Project 或所选 ExperimentPlan 分支 Project 到 `/api/mesa-analysis-runs`，返回 session 指标、表格、波次明细、日采样或事件快照。每个样本必须在独立进程中受默认 60 秒硬上限约束；整批后端预算按并行执行波次计算并限制在 180 到 900 秒，前端使用相同波次口径加 30 秒响应余量。响应必须保留请求/完成/失败样本数、逐样本诊断及分阶段耗时，前端必须区分请求超时、网络错误、整批阻断和部分样本失败。不得把这四页重新路由到 current-result 面板、AnalysisTask、MonteCarloExperiment、run/artifact 选择器、`analysis_projection_*` payload 或 `/api/runs` 账本路径。
   携行清单必须在 projection、lite Mesa 行、页面状态和 XLSX 明细中保留实际使用数量与携行总数量；总体备件利用率只按未筛选全集的总量比计算，携行量按成功样本数计入。筛选/隐藏/排序不得改变总体摘要，结果不得钳制到 100%。raw 完整且真实总携行量为 0 时页面与 XLSX summary 显示 `--`；空 projection 或 raw 缺失/非法时显示“数据不可用”，不得从 aggregate、推荐量或旧行百分比伪造总体值。
   “备件满足率下限”必须参与后端携行量约束计算，不得只回显。每个“机型 + 产品”按逐样本需求寻找满足 `Σ min(样本需求, 携行量) / Σ样本需求` 下限的最小整数携行量，利用率使用约束后预计满足量除以全部成功样本的携行总量，并在 projection、页面和 XLSX 明细中保留下限、约束后满足率、达标状态和约束余量；提高下限不得降低建议携行量。
17. ExperimentPlan config 继续携带分支 `projectJson`、samples、parallelCores、seed 和页面会话设置；只有状态为 frozen 且通过指纹校验的方案可作为已冻结运行上下文。lite Mesa 和可视化 session 必须严格使用所选冻结方案的分支 Project，禁止回退到当前 Project 或接受请求级参数覆盖。旧 RunService 编译分支 Project JSON 的能力只作为内部治理/历史账本路径保留，不是四个分析页和 Monte Carlo 详情的用户主流程。
...
27. M9.8 平台嵌入后的当前用户可见分析结果必须来自 lite Mesa 会话摘要：Monte Carlo 详情和四类分析页通过 `/api/mesa-analysis-runs` 运行，页面展示 session 指标、表格和事件摘要，不展示 run/artifact 选择器。可视化推演通过 `/api/visualization-sessions` 编译当前 Project draft 或已冻结方案快照，再由平台管理的 Solara iframe sidecar（默认 `8765`）按 session identity 直接驱动 `AircraftSupportV1Model.step()`；当前项目路径不得保存 Project，冻结路径不得读取当前 Project。该 sidecar 只是交互式可视化承载，不得恢复旧 contract provider、`independent-mesa/server.py`、`/mesa-visualization-runs`、静态 HTML 旁路或 `/api/runs` 用户主流程；`independent-mesa/GLM` 和 `independent-mesa/GPT` 源码树已移除，后续不得恢复为旁路服务或离线复现实验入口。
