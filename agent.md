...
15. M6.1/M9.4 输入一致性已形成窄闭环：`smoke` 和 `aviation_support` Scenario 必须带 `compiled_from.mapping_provenance`；`aviation_support` 单次正式 run 必须走 `RunService -> SimulationAdapter -> AviationSupportModel -> artifacts`，不能回退到 demo、contract provider 快照或前端局部推导。未知模型族或无效 Project 仍必须 fail closed，返回 failed status envelope、保留 `error.details.issues/provenance`、不伪造正式产物。
...
18. M6.2 统一 Monte Carlo / analysis profile 已形成同步本地切片，M8.0 已补齐 projection payload 消费，M9.0/M9.1 已补齐离线状态序列回放与事件追溯，M9.2 已补齐最小在线状态流和运行订阅，M9.3 已补齐最小 run lifecycle / control plane，M9.4 已补齐 `aviation_support` 单次正式执行，M9.5 已定义受治理的航空保障采样契约并解锁 `aviation_support` formal Monte Carlo：基于编译通过的 Scenario 产出 result、`monte_carlo_base`、projection artifacts 和 `visualization_state_series` artifact；单次仿真实验和 Monte Carlo 实验必须共享 `SimulationExperimentBase`，统一实验身份、方案、Scenario identity、随机种子、状态、进度、`run_id` 和 artifact 引用；Monte Carlo 实验是批量运行主账本，AnalysisTask 通过 `linkedMonteCarloExperimentId` 绑定 `mc_experiment_id`；正式 MC 必须使用 `run_type: "monte_carlo"`，不得回退到 `single`。四个结果分析页只有在运行完成、compiler provenance、对应 projection artifact 和已解析 projection payload 同时存在时才显示正式 KPI/表格/图形；Mesa 可视化页在已完成 run 存在 `visualization_state_series` artifact 且 payload 通过 `run_id + artifact_id`、schema、事件引用和 traceability 校验后显示正式离线回放和事件流。当前同步执行器的 M9.2 `GET /api/runs/{run_id}/state-stream` 从已持久化状态序列输出 `run_status`、`state_frame` 和 `artifact_ready`；在线帧只用于展示当前流状态，最终 artifact 才能解锁离线回放。M9.3 的 `POST /api/runs/{run_id}/control` 只在后端确认 `cancel`/`retry` 后改变正式 run 状态并写入 `runs.control.*` 审计；`retry` 会阻断旧 result/artifact 正式读取，run detail 只展示 pending 空 artifact manifest。未创建任务、运行失败、缺少 compiler provenance、缺少 projection artifact、缺少 state-series artifact、payload 解析失败或 retry-pending 无正式产物时不得显示正式结果/回放。生产 worker queue、真实运行中增量推送、object storage、完整 cancel/retry 基础设施、checkpoint restart 和新 auth/audit scope 只在对应阶段最小需要时纳入。
...
24. M9.0/M9.1 离线状态序列回放必须继续使用 canonical `/api/runs/{run_id}/artifacts/{artifact_id}` 下载 `visualization_state_series`，每帧必须携带 run identity、时间步、飞机/任务/资源状态、事件摘要、事件 id、指标引用和 run/result/manifest/compiled Scenario trace。前端播放、暂停、单步、重置、时间轴拖动和事件点击只控制本地回放索引，不控制后端执行器；缺少 artifact、payload 类型不匹配、run_id 不一致、事件引用不一致或 traceability 缺失时必须 fail closed，不能把契约服务 demo 或内置 demo frame 当成正式回放。
25. M9.7.3 已解锁 `aircraft_support_v1` 真实 single-run core 和 formal Monte Carlo/projection，但没有关闭最终字段覆盖：`SimulationAdapter.compile_scenario_with_gate()` 继续生成 `AircraftSupportV1Model` Scenario envelope 和独立 input payload，`SimulationAdapter.run_scenario()` / `run_monte_carlo_scenario()` 只在 unsupported 字段为空时执行 `src/spare_mvp_abm/aircraft_support_v1/`，并写出 canonical result/artifact/run chain/metrics/report/log/projection/state-series、`monte_carlo_base` 和样本失败账本。M9.7.2/M9.7.3 行为驱动字段必须在 report/log 中明确，且必须和“仅编译进入 payload”的字段分开。`components[].failureDistribution` 和 `supportNodes[].transportPolicies` 当前只编译进入 payload，真正行为消费留给 M9.7.4 coverage hardening；非空或未批准的 `supportOrganization` 仍 fail closed，RBD/RMS/k-out-of-n、周期任务细化和任务阶段/机场/任务区细化也留给 M9.7.4 coverage hardening。M9.8 平台嵌入和 `independent-mesa` 退役只能在 M9.7 single + MC + projections + coverage hardening 都稳定后进入。
25. M9.2 在线状态流必须继续使用 canonical `/api/runs/{run_id}/state-stream`，事件类型为 `run_status`、`state_frame` 和 `artifact_ready`；`state_frame` 复用 `visualization_state_series` 的帧字段和 traceability，不另造前端静态帧。原生 `EventSource` 的本地会话 token 只允许作为该 SSE 路由的 `access_token` 查询参数折中，其他受保护 API 仍使用 Bearer header。M9.3 后端运行控制必须继续使用 canonical `POST /api/runs/{run_id}/control`；`cancel`/`retry` 必须以后端确认和审计为准，`pause`、`resume`、`step`、`reset` 必须 fail closed，不得在前端伪造。M9.4 已解锁 `aviation_support` 单次正式 compiler/run/result/projection/state-series，M9.5 已解锁 `aviation_support` formal Monte Carlo；后续不得把 checkpoint restart 混入无对应 contract 的阶段。
26. M9.6 已冻结平台案例数据包、字段覆盖表和 golden fixtures：`tests/fixtures/modeling_import_project.json` 是唯一完整业务案例源；`tests/fixtures/m9_6_platform_case_export.json`、`tests/fixtures/m9_6_field_coverage.json` 和 `tests/fixtures/m9_6_expected_artifact_kinds.json` 是后续 M9.7/M9.8 的验收输入。修改 canonical import fixture、导出链路、adapter provenance 或 artifact kind 时，必须运行 `python3 scripts/export-m9-6-case-package.py --check` 和 `python3 -m unittest tests.test_m9_6_case_package -v`，不得先把 `independent-mesa` 旁路页面嵌入产品入口，也不得把 `8765` 或静态 HTML 输出描述为正式产品入口。
