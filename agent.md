...
22. Legacy run API retirement 已写入 `docs/superpowers/plans/2026-06-21-legacy-run-api-retirement.md`，但尚未执行。执行该切片时必须单独提交：先用测试锁定 `/api/simulation-runs*` 退场，再迁移前端、HTTP contract tests 和浏览器 smoke 到 canonical `/api/runs`，最后让旧路径返回 `410 legacy_run_api_retired`；不得顺手实现 worker queue、object storage、取消/重试、M8 projection payload 或 M9 state stream。
