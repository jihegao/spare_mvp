...
25. M9.7.2 已解锁 `aircraft_support_v1` 真实 single-run core，但没有关闭最终字段覆盖：`SimulationAdapter.compile_scenario_with_gate()` 继续生成 `AircraftSupportV1Model` Scenario envelope 和独立 input payload，`SimulationAdapter.run_scenario()` 只在 unsupported 字段为空时执行 `src/spare_mvp_abm/aircraft_support_v1/`，并写出 canonical result/artifact/run chain/metrics/report/log/projection/state-series。M9.7.2 行为驱动字段必须在 report/log 中明确，且必须和“仅编译进入 payload”的字段分开。`components[].failureDistribution` 和 `supportNodes[].transportPolicies` 当前只编译进入 payload，真正行为消费留给 M9.7.4 coverage hardening；非空或未批准的 `supportOrganization` 仍 fail closed，RBD/RMS/k-out-of-n、周期任务细化和 MC sweep 字段也留给 M9.7.4 coverage hardening。formal Monte Carlo/projection 属于 M9.7.3，不能在 M9.7.2 文档或代码里宣称已完成。
