...
15. M6.1/M9.4 输入一致性已形成窄闭环；`smoke` 仅保留为非正式低层测试模型，`aviation_support` 仅保留为历史 schema/fixture/sidecar 证据。正式 `/api/runs`、`BackendApi.start_simulation_run()`、前端 run shortcut 和 modeling import `compile-scenario` 入口只接受 `aircraft_support_v1`。旧正式模型族必须返回 `retired_model_family`，并在 details 中给出 `replacement_model_family: "aircraft_support_v1"`；未知模型族或无效 Project 仍必须 fail closed，返回 failed status envelope、保留 `error.details.issues/provenance`、不伪造正式产物。
...
Python 测试解释器约定：凡是运行 `tests/test_contract_server.py`、涉及 Mesa 的 unittest，或执行 `unittest discover` 全量发现测试，都必须使用 repo-local `.abm-mesa-test-env/bin/python`。不要用系统 `python3 -m unittest discover` 运行这些测试；当前系统解释器可能缺少 `mesa`，会在发现 `test_contract_server` 时失败。
