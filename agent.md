...
24. M9.0/M9.1 离线状态序列回放必须继续使用 canonical `/api/runs/{run_id}/artifacts/{artifact_id}` 下载 `visualization_state_series`，每帧必须携带 run identity、时间步、飞机/任务/资源状态、事件摘要、事件 id、指标引用和 run/result/manifest/compiled Scenario trace。`aircraft_support_v1` 的任务帧必须保留周期任务、复合任务、基本任务、要求机型/数量、波次、时长和实际执行飞机字段，供任务计划甘特视图直接消费。前端播放、暂停、单步、重置、时间轴拖动和事件点击只控制本地回放索引，不控制后端执行器；缺少 artifact、payload 类型不匹配、run_id 不一致、事件引用不一致或 traceability 缺失时必须 fail closed，不能把契约服务 demo 或内置 demo frame 当成正式回放。
25. M9.7.4 已在 `aircraft_support_v1` 真实 single-run core 和 formal Monte Carlo/projection 后关闭最终字段覆盖：`SimulationAdapter.compile_scenario_with_gate()` 继续生成 `AircraftSupportV1Model` Scenario envelope 和独立 input payload，`SimulationAdapter.run_scenario()` / `run_monte_carlo_scenario()` 通过 `src/spare_mvp_abm/aircraft_support_v1/` 写出 canonical result/artifact/run chain/metrics/report/log/projection/state-series、`monte_carlo_base` 和样本失败账本。report/log 必须声明 `m9_7_4_behavior_scope`，行为驱动字段包含 `components[].failureDistribution`、`supportNodes[].transportPolicies`、`reliabilityBlockDiagram`、RMS/k-out-of-n、周期任务、任务阶段/机场/任务区、保障活动 job DAG、库存、资源容量、Monte Carlo sweep 与 seed；`supportOrganization` 是 governance-only / 不驱动仿真字段，进入 provenance 而不再阻断 M9.6 frozen 案例。M9.6 field coverage 中 frozen 字段 unsupported 汇总必须保持 0。
...
27. M9.8 平台嵌入已完成：前端 RunIntent 默认模型族是 `aircraft_support_v1`，可视化仿真、single run、Monte Carlo 和四类分析页的正式结果必须来自 canonical `/api/runs`、`/api/runs/{run_id}/artifacts/{artifact_id}` 或 `/api/runs/{run_id}/state-stream`。任务视图必须以合并后的任务计划甘特表为主工作区，按周期性任务 / 复合任务 / 每天基本任务展示要求型号、数量、实际执行飞机和状态，不再拆成独立任务计划表和任务卡片。`scripts/start-system.sh` 不得启动 `independent-mesa/server.py`、不得依赖 `8765`，也不得把静态 HTML 输出描述为正式产品入口；`independent-mesa/GLM` 和 `independent-mesa/GPT` 只能作为历史参考、开发对照和离线复现实验。
