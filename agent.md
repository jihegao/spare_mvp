...
18. M6.2 统一 Monte Carlo / analysis profile 已形成同步本地切片，M8.0 已补齐 projection payload 消费，M9.0/M9.1 已补齐离线状态序列回放与事件追溯，M9.2 已补齐最小在线状态流和运行订阅，M9.3 已补齐最小 run lifecycle / control plane：基于 M6.1/M6.1.1 编译通过的 Scenario 产出 `monte_carlo_base`、四类 `analysis_projection_*` artifacts 和 `visualization_state_series` artifact；单次仿真实验和 Monte Carlo 实验必须共享 `SimulationExperimentBase`，统一实验身份、方案、Scenario identity、随机种子、状态、进度、`run_id` 和 artifact 引用；Monte Carlo 实验是批量运行主账本，AnalysisTask 通过 `linkedMonteCarloExperimentId` 绑定 `mc_experiment_id`；正式 MC 必须使用 `run_type: "monte_carlo"`，不得回退到 `single`。四个结果分析页只有在绑定 MC、运行完成、compiler provenance、`monte_carlo_base`、对应 projection artifact 和已解析 projection payload 同时存在时才显示正式 KPI/表格/图形；Mesa 可视化页在已完成 run 存在 `visualization_state_series` artifact 且 payload 通过 `run_id + artifact_id`、schema、事件引用和 traceability 校验后显示正式离线回放和事件流。当前同步执行器的 M9.2 `GET /api/runs/{run_id}/state-stream` 从已持久化状态序列输出 `run_status`、`state_frame` 和 `artifact_ready`；在线帧只用于展示当前流状态，最终 artifact 才能解锁离线回放。M9.3 的 `POST /api/runs/{run_id}/control` 只在后端确认 `cancel`/`retry` 后改变正式 run 状态并写入 `runs.control.*` 审计；`retry` 会阻断旧 result/artifact 正式读取，run detail 只展示 pending 空 artifact manifest。未创建任务、未绑定 MC、运行失败、缺少 compiler provenance、缺少 projection artifact、缺少 state-series artifact、payload 解析失败或 retry-pending 无正式产物时不得显示正式结果/回放。M9.4 专门解锁 `aviation_support` 正式执行，并对齐 result、projection、state-series、artifact manifest 和 run chain 后端输出。生产 worker queue、真实运行中增量推送、object storage、完整 cancel/retry 基础设施、checkpoint restart 和新 auth/audit scope 只在对应阶段最小需要时纳入。
