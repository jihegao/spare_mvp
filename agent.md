...
18. M6.2 才做统一 Monte Carlo / analysis profile：基于 M6.1/M6.1.1 编译通过的 Scenario 产出统一 MC artifacts；Monte Carlo 实验是批量运行主账本，AnalysisTask 通过 `linkedMonteCarloExperimentId` 绑定 `mc_experiment_id`，“大样本评估、备件短板、携行清单、任务可靠度、停机因素”只能作为 artifact projection 展示。M6.1/M6.1.1 不包含 Monte Carlo fan-out、官方四类 analysis artifact、worker queue、object storage、取消/重试或新的 auth/audit scope；未创建任务、未绑定 MC、运行中、运行失败或输入未通过编译时，结果分析页不得显示正式结果图表。
