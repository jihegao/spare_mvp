...
15. 后续修改仿真实验方案、Monte Carlo 运行页或结果分析页时，必须保持产品分工：ExperimentPlan 保存实验基本信息和实验类型配置；运行页显示进度与日志；方案列表显示“运行中”“已完成”“运行失败”；结果分析页按配置和 run artifacts 展示结果、运行中、失败或“未配置”，未配置页面不得显示正式结果图表。
