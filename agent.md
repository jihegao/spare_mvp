...
25. M9.2 在线状态流必须继续使用 canonical `/api/runs/{run_id}/state-stream`，事件类型为 `run_status`、`state_frame` 和 `artifact_ready`；`state_frame` 复用 `visualization_state_series` 的帧字段和 traceability，不另造前端静态帧。原生 `EventSource` 的本地会话 token 只允许作为该 SSE 路由的 `access_token` 查询参数折中，其他受保护 API 仍使用 Bearer header。订阅断线、未授权、失败和 artifact-ready handoff 必须有可见状态；暂停、单步、重置等后端运行控制属于 M9.3，不得在前端伪造。
