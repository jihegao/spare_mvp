...
internal static class GreenExtractor
{
    ...
    private static int Main()
    {
        ...
        try
        {
            ...
            Process.Start(new ProcessStartInfo(launcher) { WorkingDirectory = destination, UseShellExecute = true });
            MessageBox.Show("绿色版已解压到：\r\n" + destination + "\r\n\r\n平台正在启动。", "spare_mvp 2.0", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return 0;
        }
        catch (Exception error)
        {
            MessageBox.Show(error.Message, "绿色版解压失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return 1;
        }
    }
