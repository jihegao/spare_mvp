...
- 树父子关系是唯一纵向边来源；`supportOrganization.relations[]` 仅保存横向 DAG 边，格式为 `id`、`type: "lateral"`、`enabled`、`fromOrganizationNodeId`、`toOrganizationNodeId`、`priority`。缺失 `enabled` 迁移为 `true`；横向端点必须具有同一父节点。
