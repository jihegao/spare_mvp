...
Issue #314 将保障组织从仅用于展示/追溯的数据，收敛为可校验、可编译的 Project 契约；Issue #315 在该契约上启用本级优先、沿父链逐级上溯的纵向保障调度；Issue #316 进一步启用显式同级横向支援与纵向回退；Issue #332 收敛组织节点与运行保障点的双重身份。
...
- `supportOrganization.tree[].id` 是组织节点和运行保障点共用的唯一身份。每个运行中的组织节点恰好对应一个 `supportNodes[]` 行，且该行的 `id` 必须等于树节点 ID；规范 Project 不再保存冗余的 `supportNodes[].organizationNodeId`。
- `supportResources[].organizationNodeId`、`supportActivities[].resourceId` 与运输策略端点都引用上述组织 ID。名称仅用于显示，不再作为规范关系身份。`vertical` / `vertical_lateral` 模式下活动缺失 `resourceId` 时，只有组织树根节点恰好存在同 ID 的运行保障点才允许确定性默认并记录 provenance；否则编译阻断，不按名称、顺序或首项猜测。
...
- 历史 `supportNodes[].id`、`supportNodes[].organizationNodeId`、节点名、保障点名或节点级运输策略只在引用唯一且一致时迁移。迁移先验证旧运行 ID 与组织 ID 的一对一映射，再把 `supportNodes[].id` 改为组织 ID 并删除冗余字段；同一组织映射多个运行行、运行引用缺行、未知引用、歧义名称或冲突映射均按具体字段路径阻断。缺失策略 ID 时按规范内容生成稳定的 `migrated-transport-<hash>`，不能使用数组下标。
...
这些字段进入 consumed/derived provenance。`support_network.nodes[].id` 与 `organization_node_id` 都使用同一个规范组织 ID，`name` 仅保留显示文本；`legacy` / `vertical` 仍将 `organization_graph.lateral_edges` 标记为 `runtime_deferred_fields`，`vertical_lateral` 则把横向边作为正式行为输入。
