...
7. 进入 `可视化推演`，切换到 `Ontology视图`，确认页面显示项目级 ontology 关系图和节点汇总，而不是独立 ontology 页面或每个四级页面的右侧上下文面板。该实现记录当时采用三层汇总口径；后续设计约定已经在 `docs/ontology-mesa-rebuild-plan.md` 中更新为 `建模对象 -> 仿真实验 -> 模型实例 -> 计算产物` 四层纵向画布。
