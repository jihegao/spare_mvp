...
业务窗口加载本机工作区后将 Chromium 页面缩放恢复为 100%，不继承同一用户此前为 `127.0.0.1` 保存的站点缩放；键盘和鼠标页面缩放请求被桌面壳阻止，避免后续安装继续复用异常比例。该行为不覆盖 Windows DPI 或显示器缩放设置。
...
#383 已纳入本轮 V2 收口：蒙特卡洛及四个耗时分析入口使用后端进程内任务，切页或刷新后按用户、业务页和上下文重连同一 `task_id`；秒级的飞机任务可靠性评估保留现有前端算法并使用同一前端执行协议。任务不排队，不增加后台按钮或任务中心，也不支持退出应用或服务重启后的续算。保持唯一 Project 编译入口和当前计算/结果口径，不新增一键全部分析。#364 已完成并关闭，不恢复其历史性能任务。
