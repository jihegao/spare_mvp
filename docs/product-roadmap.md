...
M8.0 当前收束：`docs/superpowers/specs/2026-06-21-m8-projection-payload-analysis-design.md` 已把结果分析推进到 projection payload 驱动。结果分析现在按当前分析结果记录和 M6.2 run artifacts 解锁；当前页运行完成后必须下载并解析当前分析类型对应的 `analysis_projection_*` JSON payload，正式 KPI、表格和图形来自 payload adapter。当前页运行中显示进度，运行失败显示失败原因；未配置 current result、缺少 M6.1 compiler provenance、缺少 projection artifact、payload 类型不匹配、analysis type 不匹配或解析失败时统一 fail closed，不能用静态演示图表冒充正式结果。
