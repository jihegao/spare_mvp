...
- `SimulationAdapter` emits `operations_support_policy: daily-v1`, activity `plan_group_id`/`operations_phase`, and the task's resolved `operations_plan_group_id`. The task's `supportActivityName` selects its group; an omitted reference can resolve only one group whose three phases all apply to the task aircraft type. An unrestricted phase does not make a group eligible if another phase is incompatible. For composite tasks, the adapter resolves and validates each task item against `taskItems[].equipmentType` (falling back to the basic mission type) and writes its `operations_plan_group_id` on that item. All phases must apply to the effective task aircraft type; incompatible overrides are rejected by the compile gate. Multitype tasks use the existing type tokenization and require every stage to cover all selectable types; an empty stage aircraft model remains unrestricted. Runtime selection uses these identities, not display-name guesses.
