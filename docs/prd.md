...
| 文档版本 | V1.4 |
| 更新日期 | 2026-06-12 |
| 文档范围 | `备件_front` 前端原型，兼顾概要设计 CSCI 部件范围映射 |
| 参考资料 | `3概要设计方案.docx` 中“CSCI体系结构设计 / 平台部件组成 / 系统功能原理”相关章节，及 `备件截图说明文档_润色版.docx` 中“备件规划评估模块 / 任务可靠度评估模块”相关章节 |
| 当前交付形态 | 覆盖 CSCI 完整部件范围的静态前端评审原型，包含首页导航和 9 个功能页面；不代表后端、仿真或真实运行闭环已完成 |
...
当前前端原型按 CSCI 完整部件范围补齐建模、实验、结果分析和运行支持入口，参考既有“保障方案分析”类页面形态，提供模块导航、启动分析、结果展示、表格和图形化呈现能力。本轮仍是静态前端评审原型，所有结果数据均为前端内置演示数据，不代表真实仿真计算输出。

项目当前路线已经重置：之前版本的前端和建模实现均废弃。当前 PRD 以新的前端评审原型为本轮实现范围，同时补充概要设计中的完整 CSCI 部件清单，作为后续平台能力补齐、字段契约和后端仿真算法对齐的范围依据。
...
本期原型包含 3 个模块、9 个功能页面；若把首页导航也计入可访问页面，则共 10 个页面：
...
以下范围来自 `3概要设计方案.docx` 的 CSCI 部件组成，用于说明平台完整能力边界。当前静态前端已为各类 CSCI 部件补齐最小页面入口，但这些页面只覆盖前端表达，不代表后端接口、仿真内核、真实运行调度或数据持久化已经完成。

| CSCI 部件 | 二级功能 | 三级功能 | 四级功能或能力点 | 当前路线 A 覆盖情况 |
| --- | --- | --- | --- | --- |
...
1. 当前交付目标仍是静态前端评审原型，不接入后端、仿真内核、真实接口、运行调度或数据持久化。
2. 完整 CSCI 部件清单用于防止 PRD 遗漏概要设计范围；当前页面只证明前端入口和表达完整，不证明真实计算能力完整。
3. 后续若进入正式平台阶段，应优先补齐“仿真建模、仿真实验、蒙特卡洛实验、结果存储、运行支持”对应的接口、字段契约、运行产物和验收标准。
...
| 当前页面 | 所属 CSCI 部件 | 对应概要设计能力 | 覆盖方式 |
| --- | --- | --- | --- |
| 备件规划仿真建模 | 备件规划评估模块 / 仿真建模 | 任务、装备、保障组织、保障活动和指标分配方案管理 | 静态建模页 |
| 备件规划仿真实验 | 备件规划评估模块 / 仿真实验 | 实验方案管理、可视化推演、蒙特卡洛实验和结果入口 | 静态实验页 |
| 备件短板分析 | 备件规划评估模块 / 结果分析 | 利用蒙特卡洛实验产生的备件需求数据集，按备件需求量和不满足情况识别短板 | 静态结果页 |
| 转场携行清单分析 | 备件规划评估模块 / 结果分析 | 基于备件利用率、满足率和短缺风险迭代优化转场携行清单 | 静态结果页，含演示约束 |
| 任务可靠度仿真建模 | 任务可靠度评估模块 / 仿真建模 | 任务、装备故障、可靠性框图、保障组织和指标分配方案管理 | 静态建模页 |
| 任务可靠度仿真实验 | 任务可靠度评估模块 / 仿真实验 | 实验方案管理、可视化推演、蒙特卡洛实验和结果入口 | 静态实验页 |
| 任务完成度评估 | 任务可靠度评估模块 / 结果分析 | 基于波次任务成功概率和任务可靠度指标展示任务完成趋势 | 静态结果页 |
| 停机因素分析 | 任务可靠度评估模块 / 结果分析 | 对影响任务可靠度的停机因素进行识别、归因和贡献度展示 | 静态结果页 |
| 运行管理和产物检查 | 系统运行支持模块 / 仿真运行支持 | 长周期大样本运行优化、样本统计、运行日志和结果产物检查 | 静态运行支持页 |
...
| 编号 | 验收项 | 预期结果 |
| --- | --- | --- |
| AC-01 | 首页模块入口 | 展示 3 个模块、9 个功能页面入口 |
| AC-02 | 仿真建模页面 | 备件规划和任务可靠度模块均提供静态建模入口，覆盖任务、装备、保障组织、保障活动和指标分配方案管理 |
| AC-03 | 仿真实验页面 | 备件规划和任务可靠度模块均提供静态实验入口，覆盖实验方案、可视化推演、蒙特卡洛实验和结果入口 |
| AC-04 | 备件短板分析 | 仅保留启动按钮，结果展示短板表格和结论 |
| AC-05 | 转场携行清单分析 | 展示优化条件和任务可靠度、备件满足率、备件利用率三个滚轮阈值 |
| AC-06 | 转场优化条件切换 | 切换后结果指标卡同步更新 |
| AC-07 | 任务完成度评估 | 仅保留启动按钮，展示折线图和波次表 |
| AC-08 | 停机因素分析 | 仅保留启动按钮，展示因素分解和贡献度表 |
| AC-09 | 系统运行支持 | 展示长周期大样本运行优化、运行目录、日志和结果产物检查入口 |
| AC-10 | 静态边界 | 页面不承诺后端接口、真实仿真计算、真实运行调度或持久化结果已完成 |
| AC-11 | 响应式 | 1440px 桌面和 390px 移动视口无水平溢出 |
...
3. [后续待确认] 四个结果分析页面在正式系统中是否完全不需要隐藏参数，还是仅当前原型阶段不展示。
