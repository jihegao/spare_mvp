...
7. 当前用户可见分析主线为 `当前 Project 或已保存 ExperimentPlan -> POST /api/mesa-analysis-runs -> aircraft_support_v1 simulation inputs -> CompiledSimulation + in-memory CompiledAircraftSupportModel -> lite Mesa 会话摘要`；Monte Carlo 和结果分析页继续保留该 Project/plan 数据边界。方案编辑器中的 `experimentPlanDraft` 只是内存分支，保存后才创建或更新 ExperimentPlan。可视化推演页单独收敛为“实验方案”选择：下拉只列当前 Project 已保存且具有稳定 `experiment_plan_id` 的 ExperimentPlan，不提供“当前项目”或未保存草稿回退；没有方案、尚未选择或已选方案失效时不加载 Solara iframe，也不发出 Project 保存请求，并提供实验方案管理入口。旧 `/api/runs`、RunService、SimulationRun、ResultSummary 和 ArtifactManifest 运行账本路径保留为历史实现、内部治理能力或后续持久化运行治理候选；旧 contract provider、`independent-mesa` sidecar、smoke model、smoke scenarios 和 smoke JSON fixtures 已退役删除。
   ...
   Monte Carlo ExperimentPlan 编辑器同时保存 `monteCarloBackend` 与 `monteCarloOutputScope`。后端选项为 `python`（默认）和 `rust_event_time_v2`；Python 默认允许核心结果与分析模块结果，Rust event-time v2 强制 `core`，页面明确提示“仅核心 Monte Carlo 结果，不生成分析模块结果”。运行详情在字段可用时展示后端、样本数、并行度、耗时、吞吐、缓存状态和失败样本数，旧 run/detail 缺字段时显示“未提供”。分析页面收到 `analysis_status=not_generated` 时只展示“未生成”状态，不以零值冒充分析结果。
...
11a. 飞机转场携行清单的行级备件利用率按 `spare_consumed` 事件数量除以“建议携行数量 × 成功执行样本数”计算，projection 明确传递 `used_quantity` / `carried_quantity`。总体备件利用率按未筛选全集的 `Σused_quantity ÷ Σcarried_quantity × 100%` 计算，不平均行级百分比，筛选、隐藏和排序均不得改变摘要；总携行量大于 0 时显示两位百分比且不钳制到 100%，真实总携行量为 0 时显示 `--`。空 projection 或任一原始数量缺失/非法时显示“数据不可用”，不得由 aggregate、推荐量或行百分比反推总体值。行级分母大于 0 且无使用时为 `0.00%`，分母为 0 时为“不可计算”；页面与 Excel summary 使用同一总体值，Excel 明细保留实际使用数量和携行总数量。
11b. 飞机转场携行清单的“备件满足率下限”是后端约束输入，不是展示字段。每个“机型 + 产品”寻找最小整数携行量 `q`，使各成功样本需求 `dᵢ` 的预计满足量 `Σ min(dᵢ, q)` 不低于 `r × Σdᵢ`；零需求时满足率为 `1`，利用率按约束后预计满足量除以全部成功样本的携行总量计算。projection、页面和 Excel 明细统一输出约束后满足率、满足率下限、达标状态和约束余量；提高下限时建议量只能保持或增加。
