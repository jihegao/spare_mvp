...
5. RMS 页面在同一“RMS 输入与指标分配计算”面板内维护任务时长、整机 MTBF 和 MTTR；等分配法、比例分配法和相似产品分配法先确定节点权重，再统一换算失效率、MTBF 和 MTTR。节点结果与真实 XLSX 均展示层级、节点、运行比、失效率、MTBF、MTTR 六列。导入只更新 RMS 工作台独立数据，不污染项目建模数据；当前契约不包含任务可靠度、目标 R、可靠度校核、MTBCF 或 MTTR 裕度，也不提供保存草稿或发布入口。装备系统建模页的模板下载、当前机型导出和文件上传集中在左侧装备结构树面板，右侧只展示建模详情。
...
11a. 飞机转场携行清单的行级备件利用率按 `spare_consumed` 事件数量除以“建议携行数量 × 已执行样本数”计算。分母大于 0 且无使用时为 `0.00%`；分母为 0 或 projection 缺失字段时为“不可计算”；筛选后的页面与 Excel 导出使用相同行、列名和两位百分比格式。
