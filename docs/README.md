...
17. 使用保障活动建模树允许为空；一个用户创建的方案叶子在数据层关联飞行前、再次出动和飞行后三个阶段，但阶段不作为树节点展示。三个阶段的新建工作项目清单均为空；点击“新增工作项目”只加入用户从基本保障活动库明确选择的项目，不得自动生成“飞行前准备基本保障活动1”“再次出动准备基本保障活动1”或“飞行后检查基本保障活动1”。
