...
10. 其余四个结果分析页当前为独立轻量 Mesa 会话页：前端提交当前 Project 与页面设置到 `POST /api/mesa-analysis-runs`，后端编译为 `aircraft_support_v1` simulation inputs 后只在内存中运行样本并返回页面摘要；该路径不创建 `/api/runs`、SQLite run、Result 或正式 artifact，也不读取 current result 面板。四页均在“分析设定”之前显示运行上下文，并保留显式“运行分析”入口、运行状态、样本量和随机种子；备件短板、携行清单和任务可靠度页不恢复标题横幅，停机因素页恢复标题横幅并保留排序范围。界面不显示“样本量 / 随机种子只读”提示。
