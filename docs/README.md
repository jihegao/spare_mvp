...
3. 四级功能页面标题显示三级功能名称；非建模页面右上角保留当前方案名称，点击可回到对应模块的仿真实验方案列表。
...
9. 可视化推演页面直接嵌入 Mesa 航空保障可视化状态，不再显示外层“可视化实验启动与停止”标题和外层四级导航。
10. 蒙特卡洛实验配置页只保留参数配置和“启动”；启动后回到方案列表并显示“运行中”。
11. 蒙特卡洛评估结果已经迁移到“结果分析 / 蒙特卡洛实验结果展示”。
12. 两个模块的结果分析页面已对齐 `vendor/ship_front/备件_front` 的页面形态。
13. 保障组织建模、保障活动建模已对齐 `vendor/ship_front` 的树表编辑、方案列表和网络图形态。
14. Ontology Playground 导出关系 ID 已加唯一性约束；Monte Carlo 扫参输入会真实更新场景并重算结果。
