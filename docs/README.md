...
11b. 飞机转场携行清单的“备件满足率下限”是后端约束输入，不是展示字段。每个“机型 + 产品”寻找最小整数携行量 `q`，使各成功样本需求 `dᵢ` 的预计满足量 `Σ min(dᵢ, q)` 不低于 `r × Σdᵢ`；零需求时满足率为 `1`，利用率按约束后预计满足量除以全部成功样本的携行总量计算。projection 保留约束后规划满足率与下限；页面和 Excel 移除规划满足率与单独阈值，保留实际备件满足率、备件利用率、达标状态和百分点余量。提高下限时建议量只能保持或增加。
