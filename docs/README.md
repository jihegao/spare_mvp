...
5. RMS 页面按机型选择 `basicMissions[]` 中的基本任务，只读使用其任务时长，并输入整机任务可靠度 `R(T)` 和 MTTR；复合任务、周期任务及其他大周期任务剖面不参与计算。等分配法、比例分配法和相似产品分配法从整机递归分解到系统、分系统和 LRU，每个节点配置运行比，内部按 `T × 运行比` 换算指标，但不展示或保存“节点暴露”字段。装备组成/安装数与分解结果分别支持导入；分解结果可编辑、显式保存、重新打开恢复并导出真实 XLSX，所有过程数据保持在 RMS 工作台边界，不污染正式 Scenario 模型输入。
...
7. 当前用户可见分析主线为 `当前 Project 或已保存 ExperimentPlan -> POST /api/mesa-analysis-runs -> aircraft_support_v1 simulation inputs -> CompiledSimulation + in-memory CompiledAircraftSupportModel -> lite Mesa 会话摘要`；Monte Carlo 和结果分析页继续保留该 Project/plan 数据边界。方案编辑器中的 `experimentPlanDraft` 只是内存分支，保存后才创建或更新 ExperimentPlan。可视化推演页单独收敛为“实验方案”选择：下拉只列当前 Project 已保存且具有稳定 `experiment_plan_id` 的 ExperimentPlan，不提供“当前项目”或未保存草稿回退；没有方案、尚未选择或已选方案失效时不加载 Solara iframe，也不发出 Project 保存请求，并提供实验方案管理入口。旧 `/api/runs`、RunService、SimulationRun、ResultSummary 和 ArtifactManifest 运行账本路径保留为历史实现、内部治理能力或后续持久化运行治理候选；旧 contract provider、`independent-mesa` sidecar、smoke model、smoke scenarios 和 smoke JSON fixtures 已退役删除。
   ...
   后端提供已保存 Project 与 ExperimentPlan 的只读运行编译预检接口，复用正式 exporter、分支/快照解析和 `SimulationAdapter` gate，只返回 `compiled/blocked/unsupported` 与结构化字段问题，不创建快照、Scenario、run、结果或 artifact。接口和状态边界见 [`project-compile-preflight.md`](project-compile-preflight.md)。
...
9. “飞机任务可靠性评估”是独立的确定性结构可靠性页面，位于“任务可靠度评估”之前。它按飞机型号和任务剖面读取任务时长、可靠性框图及产品参数；输入变化后必须由用户点击“运行分析”才重新计算，结果只保留整机级汇总并可导出 XLSX，不复用 `mission_reliability` Mesa 样本成功率。任务可靠度评估则在摘要中保留四个 half-even 格式化结果字段并补充仿真总次数、成功次数，详情与 XLSX 保留每个样本、每一天、每个波次的成功数和计划数；曲线单独按日期和波次键汇总各有效样本已有成功率的算术平均，缺失/非法值排除；摘要保持原有全部样本口径。明细筛选仅影响页面和 Excel 明细，不改变摘要或趋势。该页与其余四个结果分析页统一使用后端 openpyxl 导出，不再保留浏览器内自建 ZIP/XLSX 实现。
10. 其余四个结果分析页当前为独立轻量 Mesa 会话页：前端提交当前 Project 与页面设置到 `POST /api/mesa-analysis-runs`，后端编译为 `aircraft_support_v1` simulation inputs 后只在内存中运行样本并返回页面摘要；该路径不创建 `/api/runs`、SQLite run、Result 或正式 artifact，也不读取 current result 面板。每个样本默认以独立进程执行并有 60 秒硬上限，整批预算按 `ceil(samples / worker_count)` 个波次计算、限制在 180 到 900 秒；前端按相同口径等待并增加 30 秒响应余量，不能把预算内的正常长任务误报为网络失败。响应必须带请求/完成/失败样本数、逐样本状态与耗时、以及编译/样本执行/聚合/投影总耗时；全部超时、部分失败和完成状态均应给出可操作提示。备件短板分析、飞机转场携行清单分析、飞机任务可靠性评估、任务可靠度评估和停机因素分析五个页面均在“分析设定”之前显示包含页面标题和运行上下文的标题横幅；四个轻量 Mesa 页面保留显式“运行分析”入口、运行状态、样本量和随机种子，停机因素页保留排序范围。飞机任务可靠性评估页只保留当前分析结果，不读取或展示既有历史快照；项目级历史快照 API 仅作为后端兼容能力保留。五页只在已有完成结果时启用“导出 Excel”，通过 `POST /api/analysis-results/export-xlsx` 导出。停机因素只提交本人任务标识和筛选条件，生成分析信息、结果摘要、因素排行和完整分表明细；其他类型继续发送当前结果快照。项目/实验方案身份在运行时绑定，任务恢复使用服务端来源信息。导出不重跑分析，任务可靠度摘要复用 `result_fields` 的四字段和 half-even 显示值并增加仿真总次数、成功次数，结果明细复用 SimulationAdapter 正式 projection 的逐样本逐波次明细；停机事件复用 `downtimeEventDisplayRow` 的中文与 `DAY_n HH:MM`。停机分析 canonical 明细与快照递归移除故障模式字段，页面和 XLSX 同样不展示故障模式；内部仿真事件仍可保留该参数供算法使用。界面不显示“样本量 / 随机种子只读”提示。
11. 轻量 Mesa 指标口径：`使用可用度(A) = N_可用飞机小时 / N_总飞机小时`，仿真每满 60 分钟对全部飞机做一次 0/1 点采样；组件故障传导到整机并导致提前返航、修复性维修、预防性维修及其缺资源等待记为不可用，其他状态记为可用，不足一小时尚无样本时结果为空；`出动架次率 = 起飞总架次 / 飞机总数 / 仿真总天数`，展示为小数；`仿真总天数` 来自编译后的仿真窗口，周期任务有显式星期排程时按最后有任务日停止，没有显式任务日时才回退整周期/重复次数或 `durationHours`；`整周期任务可靠度 = 全部计划任务均已评估且成功的实验数 / 已执行实验总数`，未评估或失败的任一计划任务都会使该次实验判为整周期失败，周期支持天、周、月等由 Project 实际配置形成的任意长度；`战备完好率 = 每天 14:00 的可用飞机数量 / 总飞机数量`，多天结果取日采样均值；`平均备件延误时间(h) = 总调运延误时间(分钟) / 60 / 备件调运次数`，用于备件短板页替代原先会被误读为缺件次数的分钟累计值。备件短板明细按“机型 + 备件类别”聚合，需求/满足/缺件数量来自对应事件数量，并允许直接从需求数量或满足率列头选择升序、降序；携行清单可从建议携行数量列头排序，有寿件说明放在列头悬浮提示中。
11a. 飞机转场携行清单按实际数量给出两个指标：即时满足率为 `Σimmediately_filled_quantity ÷ Σdemand_quantity`，备件利用率为 `Σconsumed_quantity ÷ Σcarried_quantity`。行级和总体均按数量计算，满足下限按实际即时满足率判断；跨样本保留数量并加权汇总，不平均百分比或整数化实际数量。projection 中 `projected_*` 规划字段与实际字段分离；真实零分母显示 `--`，原始数据缺失显示“数据不可用”。页面、分析详情和 Excel 使用相同口径。
11b. 飞机转场携行清单的“备件满足率下限”是后端约束输入，不是展示字段。每个“机型 + 产品”寻找最小整数携行量 `q`，使各成功样本需求 `dᵢ` 的预计满足量 `Σ min(dᵢ, q)` 不低于 `r × Σdᵢ`；零需求时满足率为 `1`，利用率按约束后预计满足量除以全部成功样本的携行总量计算。projection 保留约束后规划满足率与下限；页面和 Excel 移除规划满足率与单独阈值，保留实际备件满足率、备件利用率、达标状态和以 `%` 显示的实际约束余量。提高下限时建议量只能保持或增加。
...
13. `aircraft-pre-life-semantics.md` 固定飞机寿命前置数据、预防维修阈值、失败关闭和结果追溯语义。
...
16. 保障组织按 #314 收敛为单根组织树、横向 DAG、节点/资源唯一归属、作用域和组织端点运输策略，并编译为稳定排序的 `support_network.organization_graph`；#315 启用本级/父链纵向供给，#316 以显式 `vertical_lateral` 模式启用本级→直接横向→纵向回退，#317 补齐稳定业务事实、描述性组织摘要、迁移提示及 single/MC/lite/Solara 跨运行一致性。规范、迁移、调度顺序、观测口径与失败关闭规则见 [`support-organization-contract.md`](support-organization-contract.md)。
17. 使用保障活动建模树允许为空；一个用户显式创建的方案叶子在数据层关联飞行前、再次出动和飞行后三个阶段，但阶段不作为树节点展示。三个阶段的新建工作项目清单均为空；点击“新增工作项目”只加入用户从基本保障活动库明确选择的项目，不得自动生成“飞行前准备基本保障活动1”“再次出动准备基本保障活动1”或“飞行后检查基本保障活动1”，也不得把既有通用方案隐式物化成三条阶段活动并自动保存。

18. #385、#394–#403 的当前结果展示、分段停机、样本筛选、任务引用导出、名称事务和界面状态规则见 [`analysis-results-contract.md`](analysis-results-contract.md)。本轮不调整案例产品类型、库存或冻结方案，不包含 Windows 打包部署和 issue 关闭。
...
| [`windows-desktop-rc1.md`](windows-desktop-rc1.md) | V2.0 Windows 绿色桌面版：离线自解压、桌面快捷方式、Electron 客户端、内置原生 Python 运行时、卸载、历史候选证据和最终验收边界。 |
...
| [`analysis-results-contract.md`](analysis-results-contract.md) | 分析结果、样本筛选、停机分段、任务引用导出、实验名称及界面状态契约。 |
...
| [`project-compile-preflight.md`](project-compile-preflight.md) | 已保存 Project / ExperimentPlan 的无持久化副作用运行编译预检接口。 |
...
- [V2.0 Windows 绿色桌面版安全交付契约与证据索引](windows-desktop-rc1.md)：当前正式发布门覆盖 #379、#381、#382、#366、#384、#383 和 #385；历史候选记录只作追溯，最终结论须绑定同一个 `2.0.0` 源码与安装包完成 4700-4 原生综合验收。
