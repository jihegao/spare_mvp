...
1. 项目数据管理页已收敛为 Project 数据层入口：左侧显示项目列表，`projectInfo.isTemplate` 为真的项目显示【模板】；右侧只保留模板管理、数据概览和可折叠 Project JSON 原始数据。
   后端 Project JSON 原始数据边界会剥离当前 `aircraft_support_v1` 不消费的草稿/预览字段：`missionProfile.profileType`、`missionProfile.endCondition`、`missionProfile.repeatCycleHours`、`missionProfile.analysisRequests`、`deletedSupportResourceKeys` 和拼写错误的 `supportActivities[].requireDevices`。正式 Monte Carlo 数值配置仍迁出到 `ExperimentPlan.config.analysisRequests.largeSample`；`requiredDevices` 是实际消费字段，不属于删除项。
