...
5. RMS 页面在同一“RMS 输入与指标分配计算”面板内按“RMS 输入参数 -> 指标分配方法 -> 执行计算”维护任务可靠度、任务时长、MTBF 和 MTTR、选择方法并显示计算状态，关键故障占比不再作为输入；方法保留等分配法、比例分配法和相似产品分配法，节点结果展示本次计算采用的 MTBF/MTTR 输入快照和归一化分配份额，并可导出真实 XLSX。导入只更新 RMS 工作台独立数据，不污染项目建模数据；当前契约不包含目标 R、可靠度校核、MTBCF 或 MTTR 裕度，也不提供保存草稿或发布入口。装备系统建模页的模板下载、当前机型导出和文件上传集中在左侧装备结构树面板，右侧只展示建模详情。
