...
`reports/2026-06-19-page-revision-suggestions/README.md` 是 M6.1.1 前的页面输入收口来源。当前项目数据管理入口已改为项目列表、模板管理、数据概览和 Project JSON 原始数据；下列页面字段会影响 M6.1.1 的 `ExperimentPlan + ModelingSnapshot -> Scenario compiler` 输入口径，必须在 mapping 中标注为 `required`、`defaulted`、`derived`、`unsupported` 或 `ignored`，不能由前端静默推导为正式仿真输入：
