...
- 携行清单会话与 formal projection 逐行传递实际 `demand_quantity`、`immediately_filled_quantity`、`consumed_quantity`、`carried_quantity`，并以 `projected_*` 单独保留规划值。后端、页面和 XLSX 对未筛选全集分别计算实际即时满足率和实际利用率的总量比；不得平均百分比、整数化数量或用规划值填补实际观测。raw 完整且真实零总分母时显示 `--`；空 projection 或 raw 缺失/非法时显示“数据不可用”。
